...
function LoginForm() {
  ...
  // Helper to map technical/database errors to friendly client messages
  function getErrorMessage(errString: string): string {
    if (!errString) return "";

    const lowerErr = errString.toLowerCase();

    if (
      errString === "DatabaseError" ||
      lowerErr.includes("querysrv") ||
      lowerErr.includes("etimeout") ||
      lowerErr.includes("mongodb") ||
      lowerErr.includes("mongoserverselectionerror")
    ) {
      return "Unable to connect to authentication services. Please try again later.";
    }

    if (
      errString === "CredentialsSignin" ||
  ...
  // Combine URL error param or state error
  const displayedError = getErrorMessage(error || urlError || "");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#05150d] via-green-950 to-green-800 dark:bg-gradient-to-b dark:from-[#010e07] dark:to-[#01371b] flex items-center justify-center p-4 overflow-hidden relative">
      ...
      <div className="absolute -top-32 -left-32 h-96 w-96 rounded-full bg-[#c9ded1] dark:bg-[#616a63] opacity-5 pointer-events-none" />
      <div className="absolute -bottom-1 -right-1 h-80 w-80 rounded-full bg-[#c9ded1] dark:bg-[#616a63] opacity-5 pointer-events-none" />

      <div className="absolute bottom-0 right-0 h-full sm:w-40 lg:w-20 w-10 bg-[#07160e] dark:bg-[#7ee9a3] opacity-50 mix-blend-multiply pointer-events-none" />
      <div className="absolute bottom-0 right-0 sm:h-40 lg:h-20 h-10 w-full bg-[#07160e] dark:bg-[#5ace82] opacity-50 mix-blend-multiply pointer-events-none" />
      ...
      <div className="w-full max-w-md relative z-10">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-200 dark:hover:text-gray-300 transition-colors"
                  tabIndex={-1}
        ...
        <p className="text-center text-sm text-gray-400 mt-5">
          ...
          <Link
            href="/auth/register"
            className="text-gray-200 hover:underline font-medium relative z-20"
          >
            Apply to join →
          </Link>
        </p>
